...
OUT_DIR = JOB_DIR / "out"
OUT = OUT_DIR / "stock_scanner_dashboard.html"
JSON_OUT = OUT_DIR / "stock_scanner_dashboard.json"
...
def live_system_payload(update_prices: bool = True, full_price_update: bool = False) -> list[dict]:
    ...
    if today.weekday() >= 5 and os.environ.get("SCANNER_ALLOW_WEEKEND", "").strip() not in {"1", "true", "TRUE", "yes"}:
    ...
    for path in (str(SHARED_DIR), str(PROJECT_ROOT)):
    ...
    if update_prices:
        from price_cache import prices_dir, update_universe_cache

        print("Updating QQQ price cache from FMP...", flush=True)
        qqq_updates = update_universe_cache(universe="qqq", sleep_seconds=0.05)
        print(
            "QQQ update status: "
            + str(qqq_updates["status"].astype(str).str.split(":", n=1).str[0].value_counts().to_dict()),
            flush=True,
        )

        if full_price_update:
            print("Updating full IWM price cache from FMP...", flush=True)
            iwm_updates = update_universe_cache(universe="iwm", sleep_seconds=0.05)
        else:
            cached = sorted({p.stem.upper() for p in prices_dir("iwm").glob("*.csv")})
            if len(cached) >= 100:
                iwm_scan_tickers = cached
                print(f"Updating {len(cached)} warm IWM cache tickers from FMP...", flush=True)
                iwm_updates = update_universe_cache(
                    universe="iwm",
                    tickers=sorted(set(cached + ["IWM"])),
                    sleep_seconds=0.05,
                )
            else:
                print(
                    f"Warm IWM cache small ({len(cached)}); updating first {iwm_daily_update_count} constituents...",
                    flush=True,
                )
                iwm_updates = update_universe_cache(
                    universe="iwm",
                    limit=iwm_daily_update_count,
                    sleep_seconds=0.05,
                )
                iwm_scan_tickers = raw_iwm_tickers[:iwm_daily_update_count]
        print(
            "IWM update status: "
            + str(iwm_updates["status"].astype(str).str.split(":", n=1).str[0].value_counts().to_dict()),
            flush=True,
        )
    else:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build the live stock scanner dashboard (cloud or local).")
    parser.add_argument("--no-open", action="store_true", default=True, help="Do not open a browser (default for cloud).")
    parser.add_argument("--open-browser", action="store_true", help="Open the HTML dashboard in a browser.")
    ...
    parser.add_argument("--full-price-update", action="store_true", help="Update the full cached IWM universe instead of warm/top subset.")
    parser.add_argument("--skip-upload", action="store_true", help="Build JSON only; do not upload to GCS.")
    parser.add_argument("--allow-weekend", action="store_true", help="Allow runs on Saturday/Sunday (uses last cached session).")
    ...
    if args.allow_weekend:
        # Monkeypatch weekend guard inside live_system_payload by temporarily faking weekday via env
        os.environ["SCANNER_ALLOW_WEEKEND"] = "1"
        ...
        json.dumps(
            {
                ...
                "liveScanOk": any(bool(s.get("isLive")) for s in systems),
                "systems": systems,
            },
            indent=2,
        ),
    ...
    if args.skip_upload:
        print("Skipping GCS upload (--skip-upload).", flush=True)
        return
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Failed to build scanner dashboard: {exc}", flush=True)
        sys.exit(1)
