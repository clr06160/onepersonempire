...
const ALLOWED_RELATIVE_PATHS = new Set(['setup/env.example', 'setup/review.txt']);
const BLOCKED_FILE_NAMES = new Set(['.env', '.env.local', '.env.production', '.env.example']);
...
function isBlockedRelativePath(relativePath: string) {
  ...
  if (ALLOWED_RELATIVE_PATHS.has(lowered)) return false;
  if (BLOCKED_PATH_PREFIXES.some((prefix) => lowered.startsWith(prefix))) return true;
  ...
  if (lowered.includes('upload_scanner')) return true;
  return false;
}
...
export async function buildScannerDownloadZip() {
  ...
  for (const relativePath of manifest.dataFiles || []) {
    await addFileToZip(zip, relativePath);
  }

  for (const relativePath of manifest.setupFiles || []) {
    await addFileToZip(zip, relativePath);
  }
