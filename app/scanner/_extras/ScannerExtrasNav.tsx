...
type NavAccent = 'fun' | 'leaders' | 'flight' | 'systems' | 'default';
...
type NavLink = {
  ...
  accent?: NavAccent;
  /** Push this link to the right edge of the nav row. */
  alignEnd?: boolean;
};

/**
 * Invited viewers: main desk + charts/instructions.
 * Fun is rendered last (right side) for everyone.
...
const funLink: NavLink = { href: '/scanner/fun', label: 'Fun', accent: 'fun', alignEnd: true };
...
export default function ScannerExtrasNav({ active, theme = 'dark' }: ScannerExtrasNavProps) {
  ...
  const visibleLinks = [...links.filter((link) => !link.developerOnly || isDeveloper), funLink];
      ...
      {visibleLinks.map((link) => {
        ...
        return (
          <Link
            key={link.href}
            href={link.href}
            className={`rounded-full border px-4 py-2 text-sm font-semibold ${link.alignEnd ? 'ml-auto' : ''} ${linkClass(link.accent, isActive, isLight)}`}
          >
            {link.label}
          </Link>
        );
